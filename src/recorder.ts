...
(() => {
  ...
  // Combined advance time + wait for render in single call (reduces round-trips)
  window.__advanceTimeAndWait = (deltaMs) => {
    return new Promise(resolve => {
      if (mode !== 'virtual') {
        console.warn('[Virtual Clock] __advanceTimeAndWait called but not in virtual mode');
        resolve();
        return;
      }
      ...
      // Advance CSS animations - pause first, then seek to exact time
      const animations = document.getAnimations();
      animations.forEach(a => {
        try {
          // Pause to prevent animation from running between frames
      ...
      // Force style recalculation to apply animation changes
      void document.body.offsetHeight;
      
      // Always use RAF to let browser process animation state changes
      // This ensures smooth panning/transitions between frames
      originalRAF(() => resolve());
    });
  };
})()
...
export async function recordStory(options: RecorderOptions): Promise<RecorderResult> {
  ...
  try {
    ...
    // Track pending file writes for parallel I/O
    let pendingWrites: Promise<void>[] = [];
    const WRITE_BATCH_SIZE = 20; // Flush writes every N frames

    // Capture frames by advancing virtual time
    for (let frame = 0; frame < totalFrames; frame++) {
      // Advance virtual time and wait for render in single call (reduces round-trips)
      await page.evaluate((delta) => {
        return (window as any).__advanceTimeAndWait(delta);
      }, frameInterval);
