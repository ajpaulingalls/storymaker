...
export const SupportedSites = [WP_SITE_AJA, WP_SITE_AJE, WP_SITE_AJB, WP_SITE_AJC] as const;

export const SHORT_URL_DOMAINS = ["aja.ws", "ajb.me", "ajch.io", "aje.io", "aje.news", "aja.me"];
...
export function getSiteFromAJLink(url: string): SupportedSite | null {
  const siteMatcher = new RegExp(/https?:\/\/(([\w-]+)\.)?aljazeera\.(\w{2,3}).*/);
...
function getSiteFromProxyLink(
  ...
  const proxyMatcher = new RegExp(/https?:\/\/(([\w-]+)\.(azureedge|ajnet)\.(\w{2,3}))/);
...
function getSiteFromDevLink(url: string): SupportedSite | null {
  const harbingerMatcher = new RegExp(/https?:\/\/(\w+)\.(\w+)\.aj-harbinger\.com.*/);
...
export function getUrlParams<T extends Record<string, string> = Record<string, string>>(
  url: string,
): Partial<T> {
...
export function createResizedUrlWithGivenDimensions(url: string, width: number, height: number) {
...
export const normalizeAJUrl = (url: string) => {
  ...
  return url.replace(/^https?:\/\/(?!www\.)([^./]+\.[^./]+(?:\.[^./]+)*)/, (match, domain) => {
